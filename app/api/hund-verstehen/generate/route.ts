...
const SCHEMA = `{
  "characterIntro": "2-3 Sätze: wie der Hund tickt, aus Rasse + Verhalten",
  "characterTraits": [{"label":"z.B. Energielevel","value":"konkret, 1 Zeile"} (4-6 Einträge: Energielevel, Sozialverhalten, Lerntyp, Bindung, Reizoffenheit)],
  ...
  "bodyLanguageSignals": [{"signal":"z.B. Rute hoch & steif","bedeutung":"konkret"} (4-5 Signale, möglichst rasse-/typ-relevant)],
...
${SCHEMA.replace(/\}$/, photo ? `,\n  "photoObservation": "2-3 Sätze aus dem Foto + 1 Tipp"\n}` : `,\n  "photoObservation": null\n}`)}`;
    ...
    const dataLines = [
      ...
      `Rasse: ${breed}`,
      `Alter: ${age}`,
      problem ? `Hauptthema: ${problem}` : null,
      behaviors.length ? `Beobachtetes Verhalten: ${behaviors.join(", ")}` : null,
    ].filter(Boolean).join("\n");
