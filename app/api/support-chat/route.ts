...
1) Frag zuerst nach seiner E-Mail zur Bestätigung ("Zur Sicherheit: mit welcher E-Mail hast du bestellt?"), falls sie im Gespräch noch nicht klar ist.
2) Ruf dann das Tool "update_dog_details" mit der E-Mail und den zu ändernden Feldern auf.
3) Das Tool prüft die E-Mail. Bei Erfolg bestätige kurz und sag, dass der angepasste Plan (falls schon gekauft) gerade neu erstellt und per Mail geschickt wird.
...
WICHTIG: Ändere nur nach E-Mail-Bestätigung. Wenn die E-Mail nicht passt, ändere nichts und bitte höflich um die richtige E-Mail. Für alles andere Persönliche → WhatsApp/support@pfoten-plan.de.`;
...
async function applyUpdate(leadId: string, sessionEmail: string, input: any) {
  ...
  if (!givenEmail || !leadEmail || givenEmail !== leadEmail) {
    return { ok: false, error: "E-Mail passt nicht zur Bestellung. Bitte nenne die E-Mail, mit der du bestellt hast." };
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const tools: any = [{
      ...
      description: "Ändert die Quiz-Angaben des Kunden zu seinem Hund. Nur mit E-Mail-Bestätigung aufrufen.",
      input_schema: {
        type: "object",
        properties: {
          email: { type: "string", description: "E-Mail des Kunden zur Verifikation (Pflicht)" },
