// Generiert ein personalisiertes Trainings-Tagebuch als A4-PDF — für beliebige
// Laufzeit (4 / 12 / 24 Wochen). Passt zum Intensiv- / Empfohlen- / Entspannt-Plan.
//
// Verwendung (lokal zum Preview):
//   WEEKS=4  DOG_NAME="Luna" DOG_BREED="Labrador" DOG_AGE="3 Jahre" \
//   MAIN_PROBLEM="Leinenziehen" node generate-tagebuch-pdf.mjs
//
// Output: public/tagebuch-{WEEKS}wochen-TEST.pdf
//
...
const WEEKS        = Math.max(1, Math.min(52, Number(process.env.WEEKS) || 4));
const TOTAL_DAYS   = WEEKS * 7;

// Periode in natürlicher Sprache (für Cover/Headlines)
...
// Motivations-Zitate — 12 Stück, cyclen für längere Laufzeiten
const WEEK_QUOTES = [
...
// ========= PDF-Aufbau =========
async function main() {
  console.log(`Generiere Tagebuch (${WEEKS} Wochen / ${PERIOD}) für:`, { DOG_NAME, DOG_BREED, DOG_AGE, MAIN_PROBLEM });
  ...
  // 4 fixe Seiten (Cover, Anleitung, Baseline, Abschluss) + 2 Seiten pro Woche
  ...
  // ===== PAGE 2 — So nutzt du dein Tagebuch =====
  {
    ...
    const steps = [
      ...
      {
        t: `Nach ${PERIOD} Fortschritt vergleichen`,
  ...
  // ===== PAGES 4…3+2N — Wochen (N Wochen × 2 Seiten) =====
  for (let week = 1; week <= WEEKS; week++) {
    // ---- Linke Seite: Woche X — Tages-Tracker ----
    {
      ...
      // Motivations-Zitat (cyclt durch die WEEK_QUOTES-Liste bei >12 Wochen)
      y -= 24;
  ...
  // ===== PAGE TOTAL — Abschluss =====
  {
    ...
    let y = A4_H - 70;
    p.drawText(`Deine Reise in ${PERIOD}`, {
      x: MARGIN, y, size: 24, font: fontBold, color: DARK_BROWN,
    });
    y -= 14;
    p.drawRectangle({ x: MARGIN, y, width: 50, height: 2, color: GOLD });
  ...
  const outPath = `public/tagebuch-${WEEKS}wochen-TEST.pdf`;
  writeFileSync(outPath, pdfBytes);
  console.log(`\n✅ PDF erzeugt: ${outPath}  (${pdfBytes.byteLength.toLocaleString("de-DE")} bytes, ${TOTAL_PAGES} Seiten)`);
