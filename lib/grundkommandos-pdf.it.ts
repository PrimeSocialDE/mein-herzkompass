// lib/grundkommandos-pdf.pl.ts
//
// POLNISCHE Variante (ŁapaPlan / lang="pl") von grundkommandos-pdf.ts.
// Rendert den "Plan podstawowych komend" (Content aus grundkommandos-content.pl.ts)
// als A4-PDF via pdf-lib (server-seitig). Unterschiede zur DE-Version:
//   1) Unicode-Font (Arimo, gebuendelt in public/fonts/) via @pdf-lib/fontkit —
//      damit polnische Zeichen (ł ą ę ó ś ż ź ć ń) gerendert werden. Die DE-
//      Version nutzt StandardFonts.Helvetica (WinAnsi), das PL-Zeichen STRIPPT.
//   2) S()-Sanitizer strippt NICHT mehr Nicht-Latin-1-Zeichen.
//   3) Alle sichtbaren Labels sind polnisch.
// Das deutsche grundkommandos-pdf.ts bleibt voellig unangetastet.
...
// Arimo (Unicode) rendert polnische Zeichen direkt -> KEIN Latin-1-Strip mehr.
// Nur kosmetische Normalisierung; polnische Buchstaben bleiben erhalten.
...
export async function buildGrundkommandosPDF(
  ...
  const F = {
    regular: await doc.embedFont(readFileSync(path.join(FONT_DIR, "Arimo-Regular.ttf")), { subset: true }),
    bold: await doc.embedFont(readFileSync(path.join(FONT_DIR, "Arimo-Bold.ttf")), { subset: true }),
    italic: await doc.embedFont(readFileSync(path.join(FONT_DIR, "Arimo-Italic.ttf")), { subset: true }),
  };
  const dog = S(content.dogName || "il tuo cane");
  ...
  topBar(st.p);
  st.p.drawText("ZAMPAPLAN · EXTRA PER " + dog.toUpperCase(), { x: MARGIN, y: A4_H - 120, size: 10, font: F.bold, color: BROWN });
  st.p.drawText("Piano dei comandi per ogni giorno", { x: MARGIN, y: A4_H - 165, size: 28, font: F.bold, color: INK });
  for (const [i, ln] of wrap(S(content.subtitle || "Comandi di controllo che danno al tuo cane sicurezza ogni giorno"), F.regular, 14, CONTENT_W).entries()) {
    st.p.drawText(ln, { x: MARGIN, y: A4_H - 195 - i * 20, size: 14, font: F.regular, color: MUT });
  }
  st.p.drawText("Creato personalmente per " + dog + (breed ? " - " + breed : ""), { x: MARGIN, y: A4_H - 258, size: 12, font: F.bold, color: BROWN });
  ...
  st.y = A4_H - 360;
  const knowsLines = wrap("Cosa " + dog + " sa già: " + (content.known?.length ? content.known.join(", ") : "-"), F.regular, 11, CONTENT_W - 36);
  const focusLines = wrap("Novità introdotte: " + (neu.join(", ") || "Consolidamento e sicurezza"), F.bold, 12, CONTENT_W - 36);
  const boxH = 24 + knowsLines.length * 15 + 8 + focusLines.length * 16 + 20;
  rrect(st.p, MARGIN, st.y - boxH, CONTENT_W, boxH, 12, CREAM, BORDER);
  let by = st.y - 22;
  st.p.drawText("PERSONALIZZATO PER " + dog.toUpperCase(), { x: MARGIN + 18, y: by, size: 10, font: F.bold, color: BROWN });
  by -= 20;
  ...
  if (methode) {
    eyebrow("Metodo — funziona con ogni comando");
  ...
  // Kommandos
  eyebrow("Comandi — passo dopo passo");
  for (const c of cmds) {
    ensure(60);
    const kann = /kann/.test(c.status || "");
    heading(c.title, kann ? { text: dog + " lo sa già", fg: GREEN, bg: GREEN_BG } : { text: "Nuovo per " + dog, fg: AMBER, bg: CHIP_BG });
    if (c.intro) lines(c.intro, { size: 10.5, font: F.italic, color: MUT });
    if (c.vorbereitung) box("Preparazione", c.vorbereitung, TINT, BORDER, GOLD, BROWN);
    eyebrow("Come mostrarlo: " + dog);
    numberedSteps(c.aufbau || []);
    if (c.wenn_nicht) box("Quando " + dog + " non collabora", c.wenn_nicht, AMBER_BG, AMBER_BD, AMBER, AMBER);
    if (c.wiederholung) box("Aspettative realistiche", c.wiederholung, BLUE_BG, BLUE_BD, BLUE, BLUE);
    if (c.erfolg) box("Come riconoscere il successo", c.erfolg, GREEN_BG, GREEN_BD, GREEN, GREEN);
    if (c.fehler) box("Errore frequente", c.fehler, AMBER_BG, AMBER_BD, AMBER, AMBER);
    gap(14);
  }
  ...
  if (wenn) {
    ...
    eyebrow("Quando qualcosa non va");
    heading(wenn.title);
  ...
  if (woche) {
    ...
    st.p.drawText("VERIFICA DEL SUCCESSO", { x: MARGIN + 16, y: top - 18, size: 10, font: F.bold, color: GREEN });
    let cy = top - 38;
  ...
  pages.forEach((p, i) => {
    ...
    const meta2 = `ZampaPlan - Piano dei comandi per ogni giorno - Pagina ${i + 1}/${total}`;
