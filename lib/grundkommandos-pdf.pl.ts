...
export async function buildGrundkommandosPDF(
  ...
  topBar(st.p);
  st.p.drawText("ŁAPAPLAN · EKSTRA DLA " + dog.toUpperCase(), { x: MARGIN, y: A4_H - 120, size: 10, font: F.bold, color: BROWN });
  st.p.drawText("Plan komend na co dzień", { x: MARGIN, y: A4_H - 165, size: 28, font: F.bold, color: INK });
  for (const [i, ln] of wrap(S(content.subtitle || "Komendy kontrolne, które dają Twojemu psu pewność na co dzień"), F.regular, 14, CONTENT_W).entries()) {
    st.p.drawText(ln, { x: MARGIN, y: A4_H - 195 - i * 20, size: 14, font: F.regular, color: MUT });
  }
  st.p.drawText("Stworzony osobiście dla " + dog + (breed ? " - " + breed : ""), { x: MARGIN, y: A4_H - 258, size: 12, font: F.bold, color: BROWN });
  ...
  st.y = A4_H - 360;
  const knowsLines = wrap("Co " + dog + " już umie: " + (content.known?.length ? content.known.join(", ") : "-"), F.regular, 11, CONTENT_W - 36);
  const focusLines = wrap("Nowo wprowadzone: " + (neu.join(", ") || "Utrwalenie i pewność"), F.bold, 12, CONTENT_W - 36);
  const boxH = 24 + knowsLines.length * 15 + 8 + focusLines.length * 16 + 20;
  rrect(st.p, MARGIN, st.y - boxH, CONTENT_W, boxH, 12, CREAM, BORDER);
  let by = st.y - 22;
  st.p.drawText("DOPASOWANE DO " + dog.toUpperCase(), { x: MARGIN + 18, y: by, size: 10, font: F.bold, color: BROWN });
  by -= 20;
  ...
  if (methode) {
    eyebrow("Metoda — działa przy każdej komendzie");
  ...
  // Kommandos
  eyebrow("Komendy — krok po kroku");
  for (const c of cmds) {
    ensure(60);
    const kann = /kann/.test(c.status || "");
    heading(c.title, kann ? { text: dog + " już to umie", fg: GREEN, bg: GREEN_BG } : { text: "Nowe dla " + dog, fg: AMBER, bg: CHIP_BG });
    if (c.intro) lines(c.intro, { size: 10.5, font: F.italic, color: MUT });
    if (c.vorbereitung) box("Przygotowanie", c.vorbereitung, TINT, BORDER, GOLD, BROWN);
    eyebrow("Jak to pokazać: " + dog);
    numberedSteps(c.aufbau || []);
    if (c.wenn_nicht) box("Gdy " + dog + " nie współpracuje", c.wenn_nicht, AMBER_BG, AMBER_BD, AMBER, AMBER);
    if (c.wiederholung) box("Realistyczne oczekiwania", c.wiederholung, BLUE_BG, BLUE_BD, BLUE, BLUE);
    if (c.erfolg) box("Jak poznać sukces", c.erfolg, GREEN_BG, GREEN_BD, GREEN, GREEN);
    if (c.fehler) box("Częsty błąd", c.fehler, AMBER_BG, AMBER_BD, AMBER, AMBER);
    gap(14);
  }
  ...
  if (woche) {
    ...
    st.p.drawText("CHECK SUKCESU", { x: MARGIN + 16, y: top - 18, size: 10, font: F.bold, color: GREEN });
    let cy = top - 38;
  ...
  pages.forEach((p, i) => {
    ...
    const meta2 = `ŁapaPlan - Plan komend na co dzień - Strona ${i + 1}/${total}`;
