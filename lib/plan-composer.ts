...
const WEEK_TEMPLATES = {
  fundament: (weekNum: number, dogName: string, problemLabel: string) => ({
    ...
    fortschritt: [
      `${dogName} reagiert auf die Grund-Signale dieser Woche in 2-3 Sekunden.`,
...
function buildMonatsUebersichten(
  ...
  for (let m = 1; m <= monthsTotal; m++) {
    const phaseName =
      m === 1
        ? "Fundament"
        : m <= Math.ceil(monthsTotal * 0.66)
          ? "Steigerung"
          : "Generalisierung";
    out.push({
      monat: m,
      text: `Monat ${m} steht im Zeichen der ${phaseName}-Phase. ${dog.dogName} hat in diesem Monat die Übungen ${phaseName === "Fundament" ? "in reizarmer Umgebung" : phaseName === "Steigerung" ? "Schritt für Schritt nach draußen verlagert" : "im echten Alltag etabliert"}.\n\nWichtig: Schau nicht nur auf einzelne Tage, sondern auf den Trend. Manche Tage werden besser laufen als andere — das ist normal. Entscheidend ist die Richtung über den ganzen Monat hinweg.\n\nWenn etwas stagniert, gehe eine Stufe zurück und festige. Wenn alles glatt läuft, kannst du in den nächsten Monat starten.`,
    });
  }
  return out;
}
